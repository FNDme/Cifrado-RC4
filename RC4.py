from conversor import bytes_to_bin
...
def RC4_Console(key, data):
    ...
    for i in range(len(data)):
        ...
        print("Byte ", (i + 1), " de secuencia cifrante: Salida= S[", temp[i][1], "] = ", temp[i][0], ":\t", bytes_to_bin(temp[i][0]))
        print("Byte ", (i + 1), " de texto original: Entrada: M[", (i + 1), "] = ", data[i], ":\t\t", bytes_to_bin(data[i]))
        print("Byte ", (i + 1), " de texto cifrado, Salida= C[", (i + 1), "] = ", solution[i], ":\t\t", bytes_to_bin(solution[i]))
        print()
    return solution
